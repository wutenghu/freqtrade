...
|------|------|
| 实验日期 | 2026-05-10 |
| 实验版本 | **v2 (波动率动态调仓, 20 品种)** |
| 策略名称 | SmaVwapStrategy (v2) / SmaVwapStrategyV1 (v1) |
...
| 版本 | 变更 |
|------|------|
| v1 | 现货，仅做多，无限制持仓，10 品种，0.04% 费率 |
| **v2 (当前)** | **v1 基础 + ATR 波动率动态调仓 + 扩展到 20 品种** |
...
| 指标 | v1 (固定 100 USDT) | **v2 (ATR 动态调仓)** | 变化 |
|------|-------------------|---------------------|------|
| 总交易数 | 1,829 | 1,829 | 相同 |
...
> v2 相较 v1，在仅增加少量回撤（8.58% → 9.05%）的前提下，总利润提升 36%（+222 USDT），Sharpe 从 1.99 提升至 2.25。

---

## 按交易对分解（v1 vs v2）

| 品种 | v1 盈亏 | v2 盈亏 | 变化 | v1 胜率 | v2 ATR% |
|------|---------|---------|------|---------|---------|
...
*实验版本: v2 (Long Only, 无限制持仓, 0.04% 费率, ATR 动态调仓, 20 品种)*
